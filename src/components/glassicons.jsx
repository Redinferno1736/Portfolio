const gradientMapping = {
  grey:"linear-gradient(hsl(0, 0%, 15%), hsl(0, 0%, 30%))",
...
//   github: "linear-gradient(hsl(220, 10%, 30%), hsl(220, 10%, 20%))",
//   linkedin: "linear-gradient(hsl(220, 10%, 35%), hsl(220, 10%, 25%))",
//   email: "linear-gradient(hsl(220, 10%, 32%), hsl(220, 10%, 22%))",
};
...
const GlassIcons = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-12 grid-cols-2 md:grid-cols-4 mx-auto py-[3em] overflow-visible ${className || ""
        }`}
    >
      {items.map((item, index) => (
        ...
        >
          ...
          >
            <span className="m-auto w-[2em] h-[2em] flex items-center text-white justify-center" aria-hidden="true">
              {item.icon}
            </span>
          </span>
